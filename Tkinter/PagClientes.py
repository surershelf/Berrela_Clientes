...
fonteMenor = ("Poppins", 14)
fonteMaior = ("Poppins", 19)
fonteBigger = ("Poppins", 25)
...
def adicionarClientes():
    ...
    tipoPagamento = tk.Label(segundaTela, text="Qual o tipo (Ap, casa, terreno):", font=fonteMaior, background=colorBG,
                             fg=colorLetter)
    tipoPagamento.place(x=140, y=190)
    inputTipoPagamento = tk.Entry(segundaTela)
    inputTipoPagamento.place(x=500, y=199)

    dataVencim = tk.Label(segundaTela, text="Data de Vencimento da Parcela:", font=fonteMaior, background=colorBG,
                          fg=colorLetter)
    ...
    valorParcela = tk.Label(segundaTela, text="Valor da Parcela: R$", font=fonteMaior, background=colorBG,
                            fg=colorLetter)
    ...
    def salvar():
        try:
            with open(arq, 'a') as a:
                ...
                msg = tk.Label(segundaTela, text=f'Novo registro de {nom.title()} adicionado', font=fonteMenor,
                               background=colorLabel, fg=colorLetter)
                msg.place(x=800, y=450)
        except Exception as e:
            msg = tk.Label(segundaTela, text=f'Houve um ERRO na hora de escrever os dados: {e}', font=fonteMenor,
                           background=colorLabel, fg=colorLetter)
            msg.place(x=800, y=450)
    ...
    voltar = tk.Button(segundaTela, text="Voltar", font=fonteMaior, command=lambda: voltar_janela_anterior(janela),
                       background=colorLabel, fg=colorLetter)
    ...
    def att():
        ...
        try:
            ...
            if chave_encontrada:
                ...
                tipoPagamento = tk.Label(terceiraTela, text="Qual o tipo (Ap, casa, terreno):", font=fonteMaior,
                                         background=colorBG, fg=colorLetter)
                ...
                dataVencim = tk.Label(terceiraTela, text="Data de Vencimento da Parcela:", font=fonteMaior,
                                      background=colorBG, fg=colorLetter)
                ...
                valorParcela = tk.Label(terceiraTela, text="Valor da Parcela: R$", font=fonteMaior, background=colorBG,
                                        fg=colorLetter)
                ...
                def attsalva():
                    ...
                    msg = tk.Label(terceiraTela, text="Cadastro atualizado com sucesso", font=fonteMaior,
                                   background=colorLabel, fg=colorLetter)
                    msg.place(x=700, y=400)

                save = tk.Button(terceiraTela, text="Salvar", font=fonteMaior, command=attsalva, background=colorLabel,
                                 fg=colorLetter)
                save.place(x=800, y=500)

        except FileNotFoundError:
            msg1 = tk.Label(terceiraTela, text=f"Arquivo '{arq}' não encontrado.", font=fonteMenor,
                            background=colorLabel, fg=colorLetter)
            msg1.place(x=900, y=800)
        except Exception as e:
            msg2 = tk.Label(terceiraTela, text=f"Erro ao atualizar o arquivo: {e}", font=fonteMenor,
                            background=colorLabel, fg=colorLetter)
            msg2.place(x=900, y=800)
        finally:
            arquivo_orig.close()

    voltar = tk.Button(terceiraTela, text="Voltar", font=fonteMaior, command=lambda: voltar_janela_anterior(janela),
                       background=colorLabel, fg=colorLetter)
    voltar.place(x=30, y=30)
    pesqCliente = tk.Label(terceiraTela, text="Pesquise o Cliente:", font=fonteMaior, background=colorBG,
                           fg=colorLetter)
    pesqCliente.place(x=250, y=150)
    inputPesqCliente = tk.Entry(terceiraTela)
    inputPesqCliente.place(x=500, y=159)
    pesqButton = tk.Button(terceiraTela, text="Pesquisar", font=fonteMaior, command=att, background=colorLabel,
                           fg=colorLetter)
...
def excluirClientes():
    ...
    nomeExclui = tk.Label(quartaTela, text="Coloque o nome do cliente para excluir:", font=fonteMaior,
                          background=colorBG, fg=colorLetter)
    ...
    def excluir():
        ...
        try:
            ...
            if chave_encontrada:
                msg = tk.Label(quartaTela, text=f"Registro de '{nom}' excluído com sucesso!", font=fonteMaior,
                               background=colorLabel, fg=colorLetter)
                msg.place(x=500, y=500)
            else:
                msg = tk.Label(quartaTela,
                               text=f"Nome '{nom}' não encontrado no arquivo. Nenhum registro foi excluído.",
                               font=fonteMaior, background=colorLabel, fg=colorLetter)
                msg.place(x=500, y=500)
        except FileNotFoundError:
            msg1 = tk.Label(quartaTela, text=f"Arquivo '{arq}' não encontrado.", font=fonteMaior, background=colorLabel,
                            fg=colorLetter)
            msg1.place(x=400, y=500)
        except Exception as e:
            msg2 = tk.Label(quartaTela, text=f"Erro ao excluir o registro: {e}", font=fonteMaior, background=colorLabel,
                            fg=colorLetter)
            msg2.place(x=400, y=500)

    pesqButton = tk.Button(quartaTela, text="Pesquisar", font=fonteMaior, command=excluir, background=colorLabel,
                           fg=colorLetter)
    pesqButton.place(x=900, y=140)

    voltar = tk.Button(quartaTela, text="Voltar", font=fonteMaior, command=lambda: voltar_janela_anterior(janela),
                       background=colorLabel, fg=colorLetter)
    voltar.place(x=30, y=30)
...
def adcPagadores():
    ...
    quintaTela.title("Adicionar Pagadores")
    quintaTela.configure(bg=colorBG)

    quintaTela.grid_columnconfigure(0, weight=1)
    quintaTela.grid_rowconfigure(0, weight=0)
    quintaTela.grid_rowconfigure(1, weight=1)

    msg = tk.Label(quintaTela, text='Pagadores já Adicionados', font=fonteMaior, background=colorBG, fg=colorLetter)
    msg.grid(row=2, column=0, sticky=tk.N)

    text = tk.Text(quintaTela, height=30.0)
    text.grid(row=3, column=0, sticky=tk.NS)

    scrollbar = ttk.Scrollbar(quintaTela, orient="vertical", command=text.yview)
    ...
    nomeAdc = tk.Label(quintaTela, text="Coloque o nome para adicionar pagador:", font=fonteMaior, background=colorBG,
                       fg=colorLetter)
                ...
                for linha in linhas:
                    if nom in linha:
                        with open(arq_mes, 'a') as pago:
                            ...
                            chave_encontrada = True  # Marca a chave como encontrada
            if chave_encontrada:
                msg = tk.Label(quintaTela, text="Dado atualizado com sucesso!", font=fonteMaior, background=colorLabel,
                               fg=colorLetter)
                msg.place(x=500, y=500)
            else:
                msg = tk.Label(quintaTela,
                               text=f"Chave de busca '{nom}' não encontrada no arquivo. Nenhum registro foi atualizado.",
                               font=fonteMaior, background=colorLabel, fg=colorLetter)
                msg.place(x=500, y=500)
    ...
    pesqButton = tk.Button(quintaTela, text="Pesquisar", font=fonteMaior, command=adc, background=colorLabel,
                           fg=colorLetter)
    pesqButton.place(x=900, y=140)

    voltar = tk.Button(quintaTela, text="Voltar", font=fonteMaior, command=lambda: voltar_janela_anterior(janela),
                       background=colorLabel, fg=colorLetter)
    voltar.place(x=30, y=30)
...
def verCadastros():
    ...
    try:
        ...
        voltar = tk.Button(sextaTela, text="Voltar", font=fonteMaior, command=lambda: voltar_janela_anterior(janela),
                           background=colorLabel, fg=colorLetter)
        voltar.place(x=30, y=30)
...
def pagamento_Atrasado():
    ...
    setimaTela.title("Pagamentos Atrasados")
    ...
    msg = tk.Label(setimaTela, text='PESSOAS QUE NÃO PAGARAM ESTE MÊS', font=fonteMaior, background=colorBG,
                   fg=colorLetter)
    msg.grid(row=0, column=0, sticky=tk.N)
    ...
    try:
        ...
        arquivo_orig.close()
        arquivo_orig1.close()
    voltar = tk.Button(setimaTela, text="Voltar", font=fonteMaior, command=lambda: voltar_janela_anterior(janela),
                       background=colorLabel, fg=colorLetter)
    voltar.place(x=30, y=30)
...
def verProxPag():
    janela.withdraw()
    oitavaTela = tk.Toplevel(janela)
    oitavaTela.geometry("1920x1080")
    oitavaTela.title("Ver Pagamentos Próximos")
    oitavaTela.configure(bg=colorBG)

    oitavaTela.grid_columnconfigure(0, weight=1)
    oitavaTela.grid_rowconfigure(0, weight=0)
    oitavaTela.grid_rowconfigure(1, weight=1)

    msg = tk.Label(oitavaTela, text='Pagamentos Próximos', font=fonteMaior, background=colorBG,
                   fg=colorLetter)
    msg.grid(row=0, column=0, sticky=tk.N)

    text = tk.Text(oitavaTela, height=10.0)
    text.grid(row=1, column=0, sticky=tk.NS)

    scrollbar = ttk.Scrollbar(oitavaTela, orient="vertical", command=text.yview)
    scrollbar.grid(row=1, column=1, sticky=tk.NS)
    ...
    try:
        ...
        voltar = tk.Button(oitavaTela, text="Voltar", font=fonteMaior, command=lambda: voltar_janela_anterior(janela),
                           background=colorLabel, fg=colorLetter)
        voltar.place(x=30, y=30)
...
verProxPagamentos = tk.Button(janela, text="Ver Pagamentos Próximos", font=fonteMaior, width=25, height=1,
                              command=verProxPag, background=colorLabel, fg=colorLetter)
verProxPagamentos.place(x=500, y=600)

sair = tk.Button(janela, text="Sair  ",font=fonteMaior, width=10, height=1,
                              command=sair, background=colorLabel, fg=colorLetter)
sair.place(x=30,y=30)
